...
create policy operation_settings_select_operation_members
  on public.operation_settings
  for select
  to authenticated
  using (private.has_operation_access(operation_id));
...
begin
  ...
  if not current_settings.global_pause then
    ...
    values (
      ...
      jsonb_build_object(
        ...
        'global_pause', current_settings.global_pause
      ),
      jsonb_build_object(
        'production_enabled', false,
        'global_pause', true
  ...
  return query
  select settings.production_enabled, settings.global_pause
